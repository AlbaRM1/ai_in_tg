...
- ✅ **Web Search (Tavily)**: агентный веб-поиск через tool calling — модель сама решает, когда искать актуальную информацию
...
- Во время поиска статус-сообщение показывает `🔎 Ищу в интернете: <запрос>…`.
- Финальный ответ форматируется и нарезается для Telegram как обычно.
...
- Веб-поиск работает **только на эндпоинтах/моделях с поддержкой function/tool calling**. Если эндпоинт не поддерживает инструменты, бот автоматически откатывается на обычный ответ без поиска (graceful fallback).
- Если `TAVILY_API_KEY` **не задан**, фича просто выключена — бот работает как раньше (обычный стриминг без инструментов).
