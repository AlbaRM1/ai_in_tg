...
- ✅ **Форум-топики как сессии**: каждый топик супергруппы — это **отдельный диалог** (как «New Chat» в ChatGPT). Модель **фиксируется за сессией**, бот отправляет и **закрепляет инфо-сообщение** о выбранной модели, а также **автоматически переименовывает топик** по первому сообщению.
- ✅ **Мультимодальность**:
